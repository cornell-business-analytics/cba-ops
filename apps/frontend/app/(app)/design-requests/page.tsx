...
export default function DesignRequestsPage() {
  ...
  const completedCount = requests.filter((r) => TERMINAL_STATUSES.has(r.status)).length;
  const visible = hideCompleted ? requests.filter((r) => !TERMINAL_STATUSES.has(r.status)) : requests;
