...
export default function MemberProfilePage() {
  ...
  const getOriginalValue = (key: keyof ProfileFields): string | null => {
    if (key === "name") return membership?.user_name ?? null;
    if (key === "email") return membership?.user_email ?? null;
    return (membership as Record<string, string | null | undefined>)?.[key] ?? null;
  };
