...
const byName = (a: MemberPublic, b: MemberPublic) => a.name.localeCompare(b.name);
...
export function TeamGrid({ eboard, directors, pms, analysts, abroad }: TeamGridProps) {
  ...
  return (
    <section className="container-section py-16 space-y-16" aria-label="Team">
      ...
      <GroupSection title="Directors" members={[...directors].sort(byName)} cols="sm:grid-cols-2 lg:grid-cols-4" />
      <GroupSection title="Project Managers" members={[...pms].sort(byName)} cols="sm:grid-cols-2 lg:grid-cols-4" />
      <GroupSection title="Active Analysts" members={[...analysts].sort(byName)} cols="sm:grid-cols-2 lg:grid-cols-4" />
      <GroupSection title="Abroad" members={[...abroad].sort(byName)} cols="sm:grid-cols-2 lg:grid-cols-4" />
    </section>
  );
}
